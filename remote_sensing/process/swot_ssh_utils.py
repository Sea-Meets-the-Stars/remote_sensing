...
def plot_a_segment(ax,lon,lat,dat,title='',
    ...
    # Add the scatter plot or contour plot
    lon=lon.flatten()
    lat=lat.flatten()
    im=ax.scatter(lon, lat, c=dat, s=8, marker='s', cmap=plt.cm.bwr,
               transform=ccrs.PlateCarree(),
              vmin=vmin,vmax=vmax )
    # im = ax.contourf([lon,lat],dat,levels=np.linspace(vmin,vmax,30),cmap=plt.cm.bwr)
   # Add the colorbar
